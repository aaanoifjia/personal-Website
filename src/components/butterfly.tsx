...
function Butterfly(prop: { className: string; button?: JSX.Element }) {
  return (
    <div className={`group relative ${prop.className}`}>
      <img className="absolute rotate-[40deg] group-hover:rotate-0 transition ease-in-out" src='/butterfly-wing.png'></img>
      <img className="absolute rotate-[-40deg] group-hover:rotate-0 scale-x-[-1] transition ease-in-out" src='/butterfly-wing.png'></img>
      <div className="hidden group-hover:flex absolute w-full h-full items-center justify-center">
        ...
        <div className="w-full h-full absolute flex items-center justify-center">{prop.button}</div>
